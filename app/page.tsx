...
async function getFeaturedActivities(): Promise<Activity[]> {
  const staticFeatured = getFeatured()
  ...
  try {
    ...
    const supabaseActivities: Activity[] = (data ?? []).map(s => ({
      id: s.id,
      ...
      capacity: s.capacity,
      attendees: 0,
      host: { name: s.host_name, avatar: '', bio: s.host_bio },
    ...
    const merged = [
      ...staticFeatured.filter(a => !supabaseIds.has(a.id)),
      ...supabaseActivities,
    ]
    return merged
  } catch {
